...
class Filter {
	...
	public function match($condition, $match) {
		...
		switch($condition) {
			...
			case 'ip':
				return preg_match($match, $ip_address);
			case 'op':
		...
		if (isset($this->action)) {
			switch($this->action) {
				...
				case 'ban':
					...
					Bans::new_ban($ip_address, $this->reason, $this->expires, $this->all_boards ? false : $board['uri'], -1);
...
function do_filters(array $post, $ip_address) {
	global $config;
