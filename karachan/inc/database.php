...
class PreparedQueryDebug {
	...
	public function __construct($query) {
		...
		if ($config['debug'] && $config['debug_explain'] && preg_match('/^(SELECT|INSERT|UPDATE|DELETE) /i', $query)) { $this->explain_query = $pdo->prepare("EXPLAIN $query"); }
	}
	public function __call($function, $args) {
		global $config, $debug;
		if ($config['debug'] && $function == 'execute') {
			if ($this->explain_query) { $this->explain_query->execute() or error(db_error($this->explain_query)); }
			$start = microtime(true);
		}
		if ($this->explain_query && $function == 'bindValue') { call_user_func_array(array($this->explain_query, $function), $args); }
...
function sql_open() {
	...
	if ($config['debug']) { $start = microtime(true); }
	if (isset($config['db']['server'][0]) && $config['db']['server'][0] == ':') { $unix_socket = substr($config['db']['server'], 1); }
	...
	if (!empty($config['db']['dsn'])) { $dsn .= ';' . $config['db']['dsn']; }
	try {
		$options = array(
		...
		if ($config['debug']) { $debug['time']['db_connect'] = '~' . round((microtime(true) - $start) * 1000, 2) . 'ms'; }
...
function prepare($query) {
	...
	sql_open();
	if ($config['debug']) { return new PreparedQueryDebug($query); }
