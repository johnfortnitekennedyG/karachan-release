 ...
 *   $config['additional_javascript'][] = 'inc/scripts/jquery-ui.custom.min.js'; // Optional; if you want the form to be draggable.
 *   $config['additional_javascript'][] = 'inc/scripts/quick-reply.js';
 *
 */
...
(function() {
	...
	var show_quick_reply = function(){
				...
				if ($td.find('input[type="file"]').length) {
					if ($td.find('input[name="file_url"]').length) {
						...
						if (settings.get('show_remote', false)) {
							...
							$file_url.clone().attr('placeholder', 'Upload URL').appendTo($newRow.find('td'));
					...
					$td.find('input[type="checkbox"]').each(function() {
						if ($(this).attr('name') == 'spoiler') {
							...
							$postForm.find('input[type="file"]').parent()
								.removeAttr('colspan')
								.after($('<td class="spoiler"></td>').append(this, ' ', $('<label for="q-spoiler-image">').text('Spoiler Image')));
		...
		$postForm.find('textarea[name="body"]').removeAttr('id').removeAttr('cols').attr('placeholder', 'Comment');
