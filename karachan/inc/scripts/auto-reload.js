 ...
 * Search for "$config['boards'] = array(" within your inc/config.php.
 *
 */
...
$(document).ready(function(){
	...
	// Add an update link
	$('.bottom').prev().after("<span id='updater'><a href='#' id='update_thread' style='padding-left:10px'>[Update]</a> (<input type='checkbox' id='auto_update_status' checked> Auto) <span id='update_secs'></span></span>");
		...
		$.ajax({
			...
			success: function(data) {
				...
				if ($('#auto_update_status').is(':checked')) {
					...
					if (loaded_posts > 0)
						$('#update_secs').text(fmt("Thread updated with {0} new post(s)", [loaded_posts]));
					else
