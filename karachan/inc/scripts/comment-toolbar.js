...
*   $config['additional_javascript'][] = 'inc/scripts/comment-toolbar.js';
*/
...
var formatText = (function($){
	...
	self.rules = {
		...
		customtextcolor: {
			...
			multiline: true, 
			exclusiveline: false, 
			prefix: '[[color=FFFFFF,bold,italics]]{',
			suffix: '}'
		},
		heading: {
			text: 'Heading',
	...
	self.build_toolbars = function(){
		if (!localStorage.formatText_toolbar || localStorage.formatText_toolbar == 'false'){
			...
			$('[name="body"]').before('<div class="format-text">Editor: <a href="javascript:;" onclick="formatText.toolbar_wrap(this);">Wrap</a> - <select>'+ options +'</select></div>');
		...
		if (window.Options && Options.get_tab('general')) {
			...
			Options.extend_tab('general', '\
				<fieldset>\
					...
					<label id="formatText_keybinds"><input type="checkbox">Enable formatting keybinds</label>\
					<label id="formatText_toolbar"><input type="checkbox">Hide formatting toolbar</label>\
				</fieldset>\
			');
		} else {
			var s1 = '#formatText_keybinds', s2 = '#formatText_toolbar', e = 'click';
			$('hr:first').before('<div id="formatText_keybinds" style="text-align:right"><a class="unimportant" href="javascript:void(0)">'+ 'Enable formatting keybinds' +'</a></div>');
			$('hr:first').before('<div id="formatText_toolbar" style="text-align:right"><a class="unimportant" href="javascript:void(0)">'+ 'Show formatting toolbar' +'</a></div>');
		}
