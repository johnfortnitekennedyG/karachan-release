 ...
 *   // $config['additional_javascript'][] = 'inc/scripts/strftime.min.js';
 *   $config['additional_javascript'][] = 'inc/scripts/local-time.js';
 */
...
$(document).ready(function(){
	...
	if (window.Options && Options.get_tab('general') && window.jQuery) {
		var interval_id;
		Options.extend_tab('general', '<label id="show-relative-time"><input type="checkbox">Show relative time</label>');
		...
		// populate select with style names
		let stylesSelect = $('<select id="style-selector-dropdown"></select>');
		for(let styleName in styles) {
			let opt = $('<option></option>').val(styleName).text(styleName);
			if(styleName === selectedstyle) { opt.attr('selected', true); }
			stylesSelect.append(opt);
		}
		Options.extend_tab('general', '<label id="stylesheet-selector">Style: </label>' + stylesSelect.prop('outerHTML'));
		$('#style-selector-dropdown').on('change', function() {
			let chosen = $(this).val();
			changeStyle(chosen);
		});
