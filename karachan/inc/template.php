...
function load_twig() {
	global $twig, $config;
	$cache_dir = getcwd() . '/tmp/twig_cache/';
	$loader = new Twig\Loader\FilesystemLoader(getcwd() . '/templates');
	$loader->setPaths(getcwd() . '/templates');
	$twig = new Twig\Environment($loader, array(
		'autoescape' => false,
		'cache' => (is_dir($cache_dir) && is_writable($cache_dir)) ? new MephboardTwigCache($cache_dir) : false,
		'debug' => $config['debug'],
		'auto_reload' => $config['twig_auto_reload']
	));
	if ($config['debug']) { $twig->addExtension(new \Twig\Extension\DebugExtension()); }
	$twig->addExtension(new Mephboard());
}
...
function Element($templateFile, array $options) {
	...
	if (isset($options['body']) && $config['debug']) {
		...
		if ($config['try_smarter'] && isset($build_pages) && !empty($build_pages)) { $_debug['build_pages'] = $build_pages; }
		...
		$options['body'] .= '<h3>Debug</h3><pre style="white-space: pre-wrap;font-size: 10px;">' . str_replace("\n", '<br/>', utf8tohtml(print_r($_debug, true))) . '</pre>';
	}

	// Read the template file
	if (@file_get_contents("templates/{$templateFile}")) {
		$body = $twig->render($templateFile, $options);
		if ($config['minify_html'] && preg_match('/\.html$/', $templateFile)) { $body = trim(preg_replace("/[\t\r\n]/", '', $body)); }
		return $body;
	}
	throw new Exception("Template file '{$templateFile}' does not exist or is empty in '/templates'!");
}

class MephboardTwigCache extends Twig\Cache\FilesystemCache {
...
class Mephboard extends Twig\Extension\AbstractExtension
{
	/**
	...
	*/
	public function getFilters()
	{
		return array(
			...
			new Twig\TwigFilter('hasPerm', 'twig_hasPerm_filter'),
	...
	*/
	public function getFunctions()
	{
		return array(
			...
			new Twig\TwigFunction('getThreadFileLink', 'getThreadFileLink')
		);
	}
...
function twig_hasPerm_filter($kara_user, $permission) {
	return hasPerm($permission, $kara_user);
}
